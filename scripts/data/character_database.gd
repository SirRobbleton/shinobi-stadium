...
func load_from_csv(path: String):
	...
	if file == null:
		push_error("Failed to open CSV at %s" % path)
		return
	...
	while not file.eof_reached():
		...
		if !image_file:
			print("[CHARACTER_DB] Image not found at: " + character.image_path + " for " + character.name)
			
			# Try to find an alternative image based on character name
			var fallback_image = find_fallback_image(character.name)
			if fallback_image != "":
				print("[CHARACTER_DB] Found fallback image: " + fallback_image + " for " + character.name)
				character.image_path = fallback_image
				image_file = true
			else:
				push_error("No suitable image found for: " + character.name)
		
		# Try loading the texture to verify it works
		var texture = load(character.image_path)
		if texture == null:
			push_error("Failed to load texture for " + character.name + " from " + character.image_path)
		else:
			print("[CHARACTER_DB] Successfully loaded image for " + character.name + ": " + character.image_path)
...
func find_fallback_image(character_name: String) -> String:
	...
	print("[CHARACTER_DB] Searching for fallback image with name part: " + first_name)
	
	var dir = DirAccess.open("res://assets/characters/")
	if !dir:
		push_error("Could not access assets/characters directory")
		return ""
	...
	while file_name != "":
		if !dir.current_is_dir() and (file_name.ends_with(".png") or file_name.ends_with(".jpg")):
			...
			if file_name.to_lower().contains(first_name):
				...
				print("[CHARACTER_DB] Found potential match: " + fallback_path)
				dir.list_dir_end()
	...
	if image_files.size() > 0:
		...
		print("[CHARACTER_DB] No name match found, using random image: " + random_path)
		return random_path
